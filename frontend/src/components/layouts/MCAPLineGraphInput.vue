...
const showControls = computed(() => {
    const isLineMode = mode.value === 'graph-line-mcap-default';
    return (isInequalityMode.value && userPoints.value.length === 2) || (isLineMode && userPoints.value.length > 0);
});

const canSubmit = computed(() => {
    if (mode.value === 'graph-line-mcap-default') {
        return userPoints.value.length === 2;
    }
    ...
    <div v-if="sessionStore.currentPrompt" class="two-column-layout">
        ...
        <div class="right-column">
            <CoordinatePlane
                ...
                @drag-end="(p) => handleDragEnd(p, (val) => sessionStore.updateResponse(val))"
                @point-selected="(p) => handlePointClick(p, (val) => sessionStore.updateResponse(val))"
            ...
            <div class="controls-wrapper">
                <div class="graph-controls">
                    <div v-if="showControls && isInequalityMode" class="control-group">
                        <button
                            class="toggle-btn"
